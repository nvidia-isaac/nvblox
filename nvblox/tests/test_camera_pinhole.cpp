...
CameraPinhole getTestCamera() {
  Matrix3f K;
  K << 300.0f, 0.0f, 320.0f, 0.0f, 300.0f, 240.0f, 0.0f, 0.0f, 1.0f;
  constexpr int width = 640;
  constexpr int height = 480;
  return CameraPinhole(K, width, height);
}

TEST(CameraTest, PointProjection) {
  const CameraPinhole camera = getTestCamera();
  Vector3f P(0.0, 0.0f, 5.0f);
  Vector2f u;
  EXPECT_TRUE(camera.project(P, &u));
...
TEST(CameraTest, CenterPixel) {
  ...
  EXPECT_TRUE(
      ((u - Vector2f(camera.K()(0, 2), camera.K()(1, 2))).array().abs() <
       kFloatEpsilon)
          .all());
}
  ...
  for (int i = 0; i < kNumPoints; i++) {
    ...
    const Vector2f u_perturbation_C(
        test_utils::randomSign() *
            test_utils::randomFloatInRange(
                camera.K()(0, 2), kOffImagePlaneFactor * camera.width()),
        test_utils::randomSign() *
            test_utils::randomFloatInRange(
                camera.K()(1, 2), kOffImagePlaneFactor * camera.height()));
    const Vector2f u_C =
        Vector2f(camera.K()(0, 2), camera.K()(1, 2)) + u_perturbation_C;
    ...
    const auto rayFromPixelNoChecks = [camera](const auto& u_C) {
      return Vector3f((u_C[0] - camera.K()(0, 2)) / camera.K()(0, 0),
                      (u_C[1] - camera.K()(1, 2)) / camera.K()(1, 1), 1.0f);
    };
...
TEST(CameraTest, FrustumTest) {
  ...
  std::cout << u_C.transpose() << std::endl;
...
TEST(CameraTest, UnProjectionTest) {
  CameraPinhole camera = getTestCamera();
