...
class SimpleJson {
 ...
 private:
  static SimpleJson parseValue(const std::string& json_str, size_t& pos) {
    skipWhitespace(json_str, pos);
    if (pos >= json_str.length()) return SimpleJson{};

    char c = json_str[pos];
    if (c == '{') {
      return parseObject(json_str, pos);
    } else if (c == '[') {
      return parseArray(json_str, pos);
    } else if (c == '"') {
  ...
  static SimpleJson parseObject(const std::string& json_str, size_t& pos) {
    SimpleJson obj;
    obj.type = OBJECT;
    pos++;  // skip '{'
    skipWhitespace(json_str, pos);
    ...
    while (pos < json_str.length()) {
      ...
      SimpleJson key = parseString(json_str, pos);
      skipWhitespace(json_str, pos);
      if (pos >= json_str.length() || json_str[pos] != ':') {
        break;
      }
      ...
      SimpleJson value = parseValue(json_str, pos);
  ...
  static SimpleJson parseArray(const std::string& json_str, size_t& pos) {
    SimpleJson arr;
    arr.type = ARRAY;
    pos++;  // skip '['
    skipWhitespace(json_str, pos);
  ...
  static SimpleJson parseString(const std::string& json_str, size_t& pos) {
    SimpleJson str;
    str.type = STRING;
    pos++;  // skip '"'
    ...
    if (pos < json_str.length()) pos++;  // skip closing '"'
    return str;
  }

  static SimpleJson parseNumber(const std::string& json_str, size_t& pos) {
    SimpleJson num;
    num.type = NUMBER;
  ...
  static SimpleJson parseBoolean(const std::string& json_str, size_t& pos) {
    SimpleJson boolean;
  ...
  static SimpleJson parseNull(const std::string& json_str, size_t& pos) {
    SimpleJson null_val;
...
bool loadKeyframeMetadataCollection(
  ...
  SimpleJson json = SimpleJson::parse(json_str);
...
void KeyframeMetadata::fromJson(const SimpleJson& json) {
  ...
  try {
    ...
    LOG(FATAL) << "Failed to parse camera_params_id: " << camera_id_str;
  }
...
void DataLoader::computeZPlaneTransform() {
  ...
  T_world_to_z0_plane_.translation() =
      Eigen::Vector3f(0.0f, 0.0f, static_cast<float>(-transformed_centroid_d.z()));
