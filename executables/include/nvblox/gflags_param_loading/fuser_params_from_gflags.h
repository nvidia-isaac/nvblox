...
DEFINE_string(ground_aligned_mesh_output_path, "",
              "File in which to save the transformed surface mesh (ground "
              "plane aligned to z=0). "
              "Only used if ground plane estimation is enabled.");
...
DEFINE_string(ground_plane_output_path, "",
              "File in which to save the ground plane data in YAML format.");
...
inline void set_fuser_params_from_gflags(
  ...
  if (!gflags::GetCommandLineFlagInfoOrDie("ground_aligned_mesh_output_path")
           .is_default) {
    LOG(INFO)
        << "Command line parameter found: ground_aligned_mesh_output_path = "
        << FLAGS_ground_aligned_mesh_output_path;
    fuser_ptr->ground_aligned_mesh_output_path_ =
        FLAGS_ground_aligned_mesh_output_path;
  }
